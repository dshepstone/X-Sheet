...
(function(app) {
    ...
    function updateTemplate(newViewTemplate) {
        ...
        if (newViewTemplate === 'large') {
            ...
            if (wrapper) wrapper.style.maxWidth = '8.5in'; // More accurate for 8.5x11 letter
            document.body.style.fontSize = '8pt';
            newFrameCount = 48; // Or a suitable count for 8.5x11
            console.log("On-screen view set to: Small (8.5x11 like)");
        } else if (newViewTemplate === 'fullpage') {
            if (wrapper) wrapper.style.maxWidth = ''; // Remove max-width
            document.body.style.fontSize = '10pt';
            // Keep existing frameCount for fullpage, or reset to a default like 96
            // newFrameCount = 96; // If you want fullpage to also imply a framecount reset
            console.log("On-screen view set to: Full Page");
        } else {
            console.warn("Unknown view template:", newViewTemplate);
            // Fallback to fullpage if unknown
            if (wrapper) wrapper.style.maxWidth = '';
            document.body.style.fontSize = '10pt';
        }

        // Update the application's current view state
        ...
        if (state.frameCount !== newFrameCount) {
            ...
            // If only styles changed (e.g. fullpage to large with same framecount)
            // we might still need to notify for layout updates.
            // The 'xsheet-updated' from generateTable handles drawing layer updates.
            // If generateTable is skipped, ensure drawing layer still updates.
            document.dispatchEvent(new Event('xsheet-updated')); // Trigger for drawing layer
        }
