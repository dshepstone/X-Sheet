...
(function(app) {
    ...
    // Application state
    const state = {
        frameCount: 96, // Default frame count for "fullpage" or "large"
        projectName: 'Animation_XSheet_' + new Date().toISOString().split('T')[0],
        modified: false,
        currentViewTemplate: 'fullpage', // <<< DEFAULT TO FULLPAGE VIEW
        // currentTemplate is now effectively currentViewTemplate for on-screen display
        selectedCells: [],
        isSelecting: false,
        selectionStart: null,
    ...
    function init() {
        ...
        // state.currentViewTemplate is 'fullpage' by default from state object.
        // Sync the dropdown to this initial state.
        if (elements.templateSelector) {
            elements.templateSelector.value = state.currentViewTemplate;
        }
        updateTemplate(state.currentViewTemplate); // Apply initial "fullpage" view & default frame count
    ...
    function setupEventListeners() {
        ...
        elements.templateSelector.addEventListener('change', function () {
            updateTemplate(this.value); // Pass selected value to updateTemplate
        });
    ...
    function setupTemplateOptions() {
        // Add template selector options if they don't already exist
        const templateSelector = document.getElementById('template-selector');
        if (templateSelector) {
            // Clear existing options to avoid duplicates
            templateSelector.innerHTML = '';

            // Add the three template options
            const options = [
                { value: 'fullpage', text: 'Full Page' },
                { value: 'small', text: '8.5"x11" (48 Frames)' },
                { value: 'large', text: '11"x17" (96 Frames)' }
            ];
            ...
            // Set default value
            templateSelector.value = 'fullpage';
        ...
        // Add CSS rules for print styling
        const styleElement = document.createElement('style');
        styleElement.textContent = `
            @media print {
                /* Always use letter size (8.5x11) for printing */
                @page {
                    size: letter;
                    margin: 0.5cm;
                }
                ...
                /* Force transparency in print mode */
                .drawing-layer-container,
                .drawing-layer-container canvas {
                    background-color: transparent !important;
                }
                
                /* Critical positioning for drawing layers */
                .drawing-layer-container {
    ...
    // Add to the init function in core.js
    ...
    // Update template (size and frame count) for on-screen viewing
    function updateTemplate(newViewTemplate) {
        const wrapper = document.getElementById('xsheet-wrapper');
        const printableArea = document.getElementById('printable-area');
        let newFrameCount = state.frameCount; // By default, try to preserve frame count

        // Store the current template
        state.currentViewTemplate = newViewTemplate;

        console.log(`Updating template to: ${newViewTemplate}`);

        // If a specific template is chosen, update frame count, otherwise keep current
        if (newViewTemplate === 'large') {
            // 11x17 template
            if (wrapper) wrapper.style.maxWidth = '11in';
            if (printableArea) printableArea.style.width = '11in';
            document.body.style.fontSize = '9pt';
            newFrameCount = 96; // Default frame count for large
            console.log("On-screen view set to: Large (11x17)");
        } else if (newViewTemplate === 'small') {
            // 8.5x11 template (letter size)
            if (wrapper) wrapper.style.maxWidth = '8.5in';
            if (printableArea) printableArea.style.width = '8.5in';
            document.body.style.fontSize = '8pt';
            newFrameCount = 48; // Default frame count for small
            console.log("On-screen view set to: Small (8.5x11)");
        } else if (newViewTemplate === 'fullpage') {
            // Full page (dynamic size)
            if (wrapper) wrapper.style.maxWidth = '';
            if (printableArea) printableArea.style.width = '';
            document.body.style.fontSize = '10pt';
            // For fullpage, keep existing frameCount or use a default
            console.log("On-screen view set to: Full Page");
        } else {
            console.warn("Unknown view template:", newViewTemplate);
            // Fallback to fullpage
            if (wrapper) wrapper.style.maxWidth = '';
            if (printableArea) printableArea.style.width = '';
            document.body.style.fontSize = '10pt';
        }
        ...
        if (state.frameCount !== newFrameCount) {
            state.frameCount = newFrameCount;
            elements.frameCountInput.value = newFrameCount;
            generateTable(state.frameCount); // This dispatches 'xsheet-updated'
        } else {
            // If only styles changed, still notify for layout updates
            document.dispatchEvent(new Event('xsheet-updated'));
        }

        updateStatusMessage('View template switched to ' + newViewTemplate);

        // Notify other modules of the change
        document.dispatchEvent(new CustomEvent('xsheet-template-changed', {
            detail: { template: newViewTemplate, frameCount: state.frameCount }
        }));
    }
