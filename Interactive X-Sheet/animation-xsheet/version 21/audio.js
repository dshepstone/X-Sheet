...
(function (app) {
    ...
    function renderWaveform() {
        ...
        // No need to call removeScrubListeners() here explicitly if managed on clear/destroy
        ...
        if (!table || !firstCell || !xsheetTableBody || !printableArea) {
            console.error("RenderWaveform: Missing required elements."); return;
        }
        ...
        requestAnimationFrame(() => {
            ...
            if (totalHeight <= 0 || cellWidth <= 0) {
                console.warn("RenderWaveform (rAF): Invalid dimensions. Table might be hidden."); return;
            }
            ...
            activeWaveformContainer.style.zIndex = '4';
            ...
            const scrubOverlayForEvents = overlay; // Alias for clarity in event handlers

            overlay.addEventListener('mousedown', (e) => {
                if (!state.audioBuffer || e.button !== 0 || !isScrubbingAllowed(e.target)) return;
                isScrubbing = true; // Set module-level flag
                console.log("Scrub: mousedown (overlay) -> isScrubbing = true");
                if (state.isPlaying) pauseAudio();
                if (!state.audioContext || state.audioContext.state !== 'running') { state.audioContext?.resume(); }
                updateScrubPosition(e, scrubOverlayForEvents);
            ...
            overlay.addEventListener('pointerdown', (e) => {
                ...
                if (state.isPlaying) pauseAudio();
                if (!state.audioContext || state.audioContext.state !== 'running') { state.audioContext?.resume(); }
                try { scrubOverlayForEvents.setPointerCapture(e.pointerId); } catch (err) { }
                updateScrubPosition(e, scrubOverlayForEvents);
                e.preventDefault(); e.stopPropagation();
            });
            ...
            overlay.addEventListener('click', (e) => {
                ...
                const rect = overlay.getBoundingClientRect(); const y = e.clientY - rect.top; const p = Math.max(0, Math.min(1, y / rect.height));
                const timePoint = p * state.audioBuffer.duration;
                elements.audioScrubber.value = (p * 100); state.startOffset = timePoint;
                if (state.isPlaying) { stopAudio(); playAudio(); } else { updateFrameMarker(); }
            });
            overlay.addEventListener('contextmenu', (e) => {
                e.preventDefault(); if (!state.audioBuffer) return;
                const rect = overlay.getBoundingClientRect(); const y = e.clientY - rect.top; const p = Math.max(0, Math.min(1, y / rect.height));
                showPhoneticInput(p * state.audioBuffer.duration); return false;
            });

            printableArea.appendChild(activeWaveformContainer);
            requestUpdateWaveformOverlayPosition();
    ...
    function updateWaveformOverlayPosition() {
        // ... (Keep the corrected updateWaveformOverlayPosition from the previous response, ensuring parent is printableArea) ...
        if (!activeWaveformContainer || !state.audioBuffer) return; updatePositionRAFHandle = null;
        const table = document.getElementById('xsheet-table'); const firstCell = table ? table.querySelector('.waveform-col[data-frame="1"]') : null;
        const printableArea = activeWaveformContainer.parentElement; // Should be #printable-area
        if (!table || !firstCell || !printableArea) { if (activeWaveformContainer) activeWaveformContainer.style.display = 'none'; return; }
        if (firstCell.offsetWidth === 0 || firstCell.offsetHeight === 0) { if (activeWaveformContainer) activeWaveformContainer.style.display = 'none'; return; }
        activeWaveformContainer.style.display = 'block';

        const cellRect = firstCell.getBoundingClientRect();
        const printableAreaRect = printableArea.getBoundingClientRect(); // Get parent rect
        const xsheetTableBody = document.getElementById('xsheet-body'); if (!xsheetTableBody) return; const totalRows = xsheetTableBody.querySelectorAll('tr[data-frame]').length; const rowHeight = firstCell.offsetHeight; const totalHeight = totalRows * rowHeight;

        // Calculate position relative to printableArea
        const relativeCellLeft = cellRect.left - printableAreaRect.left;
        const relativeCellTop = cellRect.top - printableAreaRect.top;
        const targetLeft = relativeCellLeft - 1; const targetTop = relativeCellTop;

        const currentLeft = parseFloat(activeWaveformContainer.style.left) || 0; const currentTop = parseFloat(activeWaveformContainer.style.top) || 0; let positionChanged = false; if (Math.abs(currentLeft - targetLeft) > 0.5) { activeWaveformContainer.style.left = targetLeft + 'px'; positionChanged = true; } if (Math.abs(currentTop - targetTop) > 0.5) { activeWaveformContainer.style.top = targetTop + 'px'; positionChanged = true; }
        let sizeChanged = false; if (activeWaveformContainer.style.width !== cellRect.width + 'px') { activeWaveformContainer.style.width = cellRect.width + 'px'; sizeChanged = true; } if (activeWaveformContainer.style.height !== totalHeight + 'px') { activeWaveformContainer.style.height = totalHeight + 'px'; sizeChanged = true; }
        const canvas = activeWaveformContainer.querySelector('.waveform-canvas'); const overlay = activeWaveformContainer.querySelector('.waveform-overlay'); let canvasResized = false;
        if (canvas) { if (canvas.width !== cellRect.width || canvas.height !== totalHeight) { canvas.width = cellRect.width; canvas.height = totalHeight; canvasResized = true; } }
        if (overlay && sizeChanged) { overlay.style.height = totalHeight + 'px'; }
        if (canvasResized && canvas) { drawWaveformOnCanvas(canvas, state.waveformData, state.audioBuffer.duration); }
        if (positionChanged || sizeChanged || canvasResized) { renderPhoneticMarkers(); updateFrameMarker(); }
    }
