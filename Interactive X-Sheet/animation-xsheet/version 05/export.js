...
(function(app) {
    ...
    function prepareDrawingLayersForExport() {
        ...
        // 3) Make the container a direct child of the printable area for proper positioning
        const printableArea = document.getElementById('printable-area');
        printableArea.appendChild(layerContainer);

        // Initial styling to prevent flickering
        layerContainer.style.position = 'absolute';
        layerContainer.style.zIndex = '20';

        // Don't set exact positioning yet - will be done in recalculateDrawingLayerPosition
    ...
    function recalculateDrawingLayerPosition() {
        ...
        // This is critical for print alignment
        const relativeTop = tableRect.top - printableRect.top;
        const relativeLeft = tableRect.left - printableRect.left;

        // Position the drawing layer container to exactly match the table
        layerContainer.style.position = 'absolute';
        layerContainer.style.top = relativeTop + 'px';
        layerContainer.style.left = relativeLeft + 'px';
        ...
        // Also update each canvas layer to match dimensions
        layerSystem.layers.forEach(layer => {
            if (layer.canvas) {
                // Preserve drawings by saving current content
