...
(function (app) {
    ...
    class DrawingLayerSystem {
        ...
        init() {
            ...
            const xsheetTable = document.getElementById('xsheet-table'); // Or this.xsheetTable
            if (xsheetTable) {
                // Position container to cover only the table area, relative to printableArea
                this.container.style.top = xsheetTable.offsetTop + 'px';
                this.container.style.left = xsheetTable.offsetLeft + 'px';
                this.container.style.width = xsheetTable.offsetWidth + 'px';
                this.container.style.height = xsheetTable.offsetHeight + 'px';
            } else {
                // Fallback if table not found
                this.container.style.inset = '0'; // Covers parent
        ...
        // drawing.js - inside DrawingLayerSystem class

        // Modify updateLayoutSize to optionally accept dimensions, 
        // or to always re-measure if no dimensions are passed.
        updateLayoutSize(newWidth, newHeight) {
            if (!this.xsheetTable || !this.container) return;
            ...
            if (newWidth !== undefined && newHeight !== undefined) {
                ...
                // If no dimensions passed, measure from the DOM (for screen view restoration)
                const tableRect = this.xsheetTable.getBoundingClientRect();
                // Get printableArea's rect to correctly calculate offset position IF NEEDED for screen view
                // const printableRect = this.container.parentNode.getBoundingClientRect(); 
                ...
                // For screen view, it's usually simpler if drawing.js positions relative to xsheetTable's direct parent
                this.container.style.top = this.xsheetTable.offsetTop + 'px';
                this.container.style.left = this.xsheetTable.offsetLeft + 'px';

                tableWidth = this.xsheetTable.offsetWidth;
                tableHeight = this.xsheetTable.offsetHeight;
            }

            this.container.style.width = tableWidth + 'px';
            this.container.style.height = tableHeight + 'px';

            this.layers.forEach(layer => {
                if (layer.canvas) { // Ensure canvas exists
                    layer.canvas.width = tableWidth;
                    layer.canvas.height = tableHeight;
                }
            });
    ...
    class DrawingToolSystem {
        ...
        createToolbar() {
            ...
            clearButton.addEventListener('click', () => {
                ...
                if (confirm('Are you sure you want to clear all drawings on all layers? This cannot be undone.')) {
                    this.layerSystem.clearAllLayers();
                    // Optionally, also notify that the sheet is modified if drawings were present
                    if (window.XSheetApp && window.XSheetApp.state) {
                        window.XSheetApp.state.modified = true;
                        if (window.XSheetApp.updateStatusMessage) {
                            window.XSheetApp.updateStatusMessage('All drawings cleared. Project modified.');
                        }
                    }
                }
            });
    ...
    // Function to ensure drawing system is initialized properly
    function initializeDrawingSystem() {
        // Make sure the namespace exists
        window.XSheetApp = window.XSheetApp || {};
        window.XSheetApp.Drawing = window.XSheetApp.Drawing || {};

        // Check if drawing system already initialized
        if (window.XSheetApp.Drawing.state && window.XSheetApp.Drawing.state.initialized) {
            console.log("Drawing system already initialized");
            return;
        }

        // Add temporary init function if it's not already defined
        if (!window.XSheetApp.Drawing.init) {
            // This is a fallback in case the drawing.js file hasn't loaded yet
            window.XSheetApp.Drawing.init = function () {
                console.log("Temporary init function for drawing system");
                const drawingToolbarContainer = document.getElementById('drawing-toolbar-container');

                if (drawingToolbarContainer) {
                    // Create a placeholder until the real drawing system loads
                    drawingToolbarContainer.innerHTML = `
                    <div class="drawing-toolbar" style="display: flex; flex-wrap: wrap; gap: 5px; padding: 10px; 
                         background-color: #f5f5f5; border-radius: 5px; justify-content: center;">
                ...
                `;
                    ...
                    // Add a retry mechanism for the real drawing system
                    setTimeout(function () {
                        if (typeof DrawingLayerSystem === 'undefined') {
                            // Real drawing system still not loaded, add a message
                            console.log("Waiting for drawing system to load...");
                        }
                    }, 1000);
                }
            };
        ...
        // Try to initialize the drawing system
        if (window.XSheetApp.Drawing.init) {
            window.XSheetApp.Drawing.init();
        }
    }

    // Initialize drawing system when the page loads
    document.addEventListener('DOMContentLoaded', function () {
        ...
        setTimeout(initializeDrawingSystem, 500);

        // And once more after a longer delay as a fallback
        setTimeout(initializeDrawingSystem, 1500);
    });

    // Also try to initialize when the core app is initialized
    document.addEventListener('xsheet-initialized', function () {
        initializeDrawingSystem();
    });
...
// Position relative to canvas
    ...
    // ImageObject (used by ImageTool but not in the original code)
    class ImageObject extends DrawingObject {
        ...
        loadImage() {
            ...
            this.image.onload = () => {
                this.loaded = true;
                // If no width/height specified, use the image's natural dimensions
                if (this.width === 0) this.width = this.image.naturalWidth;
                if (this.height === 0) this.height = this.image.naturalHeight;
                
                // Request redraw
                if (window.XSheetApp && window.XSheetApp.Drawing && window.XSheetApp.Drawing.state.layerSystem) {
                    window.XSheetApp.Drawing.state.layerSystem.redrawAll();
                }
            };
    ...
    // Add ImageObject to factory
    DrawingObjectFactory.types['image'] = ImageObject;
