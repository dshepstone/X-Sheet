...
(function (app) {
    ...
    function exportToPDF() {
        ...
        setTimeout(() => {
            ...
            }, 200); // Increased delay for final style application
        }, 600); // Increased initial delay for print style application
    }
    ...
    function printSheet() {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                // Additional fallback timeout for Firefox (fires afterprint earlier)
                setTimeout(() => {
                    recalculateDrawingLayerPosition();
                }, 200);
    ...
    // Prepare drawing layers for export - UPDATED
    function prepareDrawingLayersForExport() {
        ...
        // 2) Create a print-specific clone of all drawing layers
        const printContainer = document.createElement('div');
        printContainer.className = 'print-drawing-container';
        ...
        // Initial positioning - will be recalculated after print styles are applied
        const printableArea = document.getElementById('printable-area');
        printableArea.appendChild(printContainer);

        // Store reference to the print container
        layerSystem._printContainer = printContainer;

        // Set flag to track that print clone is active (for cleanup)
        layerSystem._isPrintCloneActive = true;

        // Hide the original layer container during print
        layerContainer.style.display = 'none';
    }

    // Recalculate drawing layer position - UPDATED
    function recalculateDrawingLayerPosition() {
        ...
        const layerSystem = app.Drawing.state.layerSystem;
        const printContainer = layerSystem._printContainer;
        const table = document.getElementById('xsheet-table');

        if (!table || !printContainer) return;

        // Get precise position of the table within the printable area
        const printableArea = document.getElementById('printable-area');
        const printableRect = printableArea.getBoundingClientRect();
        const tableRect = table.getBoundingClientRect();

        // Calculate position relative to printable area
        const relativeTop = tableRect.top - printableRect.top;
        const relativeLeft = tableRect.left - printableRect.left;

        // Position the print container to exactly match the table
        printContainer.style.position = 'absolute';
        printContainer.style.top = relativeTop + 'px';
        printContainer.style.left = relativeLeft + 'px';
        printContainer.style.width = tableRect.width + 'px';
        printContainer.style.height = tableRect.height + 'px';

        // Force reflow to ensure positioning is applied
        printContainer.offsetHeight;
    }

    // NEW FUNCTION: Clean up drawing layers after print
    function cleanupDrawingLayersAfterPrint() {
        ...
        if (layerSystem._printContainer && layerSystem._printContainer.parentNode) {
            layerSystem._printContainer.parentNode.removeChild(layerSystem._printContainer);
            delete layerSystem._printContainer;
        }

        // Restore the original layer container
        ...
        delete layerSystem._isPrintCloneActive;
    }
    ...
    // Clean up after PDF export - UPDATED
    function cleanupAfterExport(savedData) {
        // Restore UI controls
        showUIControls();

        // Check if print clone is active and clean it up
        if (app.Drawing && app.Drawing.state.layerSystem &&
            app.Drawing.state.layerSystem._isPrintCloneActive) {
            cleanupDrawingLayersAfterPrint();
        } else {
            // Fall back to original method if clone wasn't created
    ...
    // Clean up after printing - UPDATED
    ...
    // Restore drawing layers to original state - KEEP FOR BACKWARD COMPATIBILITY
