...
(function (app) {
    ...
    class DrawingLayerSystem {
        ...
        /**
 * Add this function to drawing.js in the DrawingLayerSystem class
 */
        renderDrawingsIntoCells() {
            // Get the table and all its cells
            const table = document.getElementById('xsheet-table');
            const cells = table.querySelectorAll('td');

            // Store a flag to indicate we've rendered into cells
            this._cellsRendered = true;

            // For each cell, render any drawings that overlap it
            cells.forEach(cell => {
                // Skip cells that aren't appropriate for drawing (like frame numbers)
                if (cell.classList.contains('frame-number')) return;
                ...
                // Calculate cell position relative to the drawing container
                const cellRect = cell.getBoundingClientRect();
                const containerRect = this.container.getBoundingClientRect();

                // Calculate offsets
                const offsetX = cellRect.left - containerRect.left;
                const offsetY = cellRect.top - containerRect.top;

                // Clear canvas with transparent background
                ctx.clearRect(0, 0, canvas.width, canvas.height);

                // Track if we've drawn anything to this cell
                let hasDrawings = false;

                // Draw each layer's content that overlaps this cell
                this.layers.forEach(layer => {
                    if (!layer.visible) return;

                    // Set up clipping for cell boundaries
                    ctx.save();

                    // Draw overlapping objects
                    layer.objects.forEach(obj => {
                        // Get object bounds to check for overlap
                        const bounds = obj.getBounds();

                        // Check if object overlaps this cell
                        const objLeft = bounds.x;
                        const objRight = bounds.x + bounds.width;
                        const objTop = bounds.y;
                        const objBottom = bounds.y + bounds.height;

                        const cellLeft = offsetX;
                        const cellRight = offsetX + cell.offsetWidth;
                        const cellTop = offsetY;
                        const cellBottom = offsetY + cell.offsetHeight;

                        // Check for overlap
                        if (objRight >= cellLeft && objLeft <= cellRight && objBottom >= cellTop && objTop <= cellBottom) {
                            // This object overlaps the cell - draw it with translation
                            ctx.save();
                            ctx.translate(-offsetX, -offsetY); // Adjust to cell's coordinate system
                ...
                // Only add the canvas if we actually drew something
                if (hasDrawings) {
                    // Clear cell and add the canvas
                    cell.innerHTML = '';
         ...
         * Add this function to drawing.js in the DrawingLayerSystem class
         */
        restoreDrawingsFromCells() {
            // Skip if no cells were rendered
            if (!this._cellsRendered) return;

            // Find all cells with embedded drawing canvases
            const cells = document.querySelectorAll('td[data-original-content]');

            // Restore original content to each cell
            cells.forEach(cell => {
            ...
            // Reset the flag
            this._cellsRendered = false;

            // Ensure drawing layers are visible again
            this.layers.forEach(layer => {
        ...
        init() {
            ...
            this.container.style.zIndex = '20'; // Higher z-index than the waveform overlay
        ...
        updateLayoutSize() {
            ...
            const printableRect = document.getElementById('printable-area').getBoundingClientRect();
            ...
            // Update container position to match table
            this.container.style.top = (tableRect.top - printableRect.top) + 'px';
            this.container.style.left = (tableRect.left - printableRect.left) + 'px';
            this.container.style.width = tableRect.width + 'px';
            this.container.style.height = tableRect.height + 'px';
        ...
        prepareForPrint() {
            ...
            // Force a redraw to ensure content is current
            this.redrawAll();
        }

        /**
  * Scale layer canvases during print process
  * @param {Number} scaleX - X scale factor 
  * @param {Number} scaleY - Y scale factor
  */
    scaleForPrint(scaleX, scaleY) {
        this.layers.forEach(layer => {
    ...
    /**
     * Restores normal layer appearance after print/export
     */
    cleanupAfterPrint() {
        ...
        // Force a redraw to ensure content is displayed correctly
        this.redrawAll();
    }
        enableDrawing() {
            // Enable pointer events on the canvas elements
            this.layers.forEach(layer => {
                layer.canvas.style.pointerEvents = 'auto';
            });
            // Set container's pointer-events to auto to ensure events are captured
    ...
    class SelectTool extends DrawingTool {
        ...
        activate() {
            this.active = true;
            // For select tool, we need events on the container
            this.layerSystem.container.style.pointerEvents = 'auto';
            // Also enable pointer events on canvas elements 
            // to ensure interaction with drawing objects
