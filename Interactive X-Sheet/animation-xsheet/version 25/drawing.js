...
(function (app) {
    ...
    class DrawingLayerSystem {
        ...
        // Add this to DrawingLayerSystem's init method in drawing.js
        // Replace or modify the container positioning code
        ...
        init() {
            ...
            // Critical positioning settings - USING MUCH HIGHER Z-INDEX
            this.container.style.position = 'absolute';
            this.container.style.zIndex = '9999'; // GREATLY INCREASED z-index value for visibility
            this.container.style.pointerEvents = 'none'; // Will be enabled for specific tools
            this.container.style.backgroundColor = 'transparent';
            this.container.style.display = 'block';
            this.container.style.visibility = 'visible'; // Ensure visibility
            this.container.style.opacity = '1'; // Full opacity
            ...
            // Force initial update and redraw
        ...
        // Fix for enabling drawing - replace this in DrawingLayerSystem
        enableDrawing() {
            // This is critical - enable pointer events for active layer's canvas
            const activeLayer = this.getActiveLayer();
            if (activeLayer && activeLayer.canvas) {
                activeLayer.canvas.style.pointerEvents = 'auto';
            }

            // For Select tool, we need the container to have pointer events
            // Modify the container only when needed (select tool will handle this)
        }

        // Fix for tool activation - update SelectTool's activate method
        // Replace this method in SelectTool class
        activate() {
            console.log(`Activating ${this.constructor.name}`);
            this.active = true;

            // Debug the layer system
            if (!this.layerSystem) {
                console.error("No layer system available for tool activation!");
                return;
            }

            if (!this.layerSystem.container) {
                console.error("No layer container available! Drawing will not work.");
                return;
            }

            // Make sure the layer system is properly visible
            this.layerSystem.container.style.display = 'block';
            this.layerSystem.container.style.visibility = 'visible';
            this.layerSystem.container.style.opacity = '1';

            // Enable drawing - tool-specific code will determine exact behavior
            this.layerSystem.enableDrawing();
            this.attachEvents();

            console.log(`Tool ${this.constructor.name} activated`);
        }
        ...
        addLayer(name) {
            ...
            // Ensure transparency is explicitly set in CSS
            canvas.style.backgroundColor = 'transparent';
            this.container.appendChild(canvas);

            // IMPORTANT: Always create context with alpha:true for transparency
            ...
            const layer = {
                name: name,
                canvas: canvas,
                context: ctx, // Use the context with alpha:true
                objects: [],
                visible: true
            };
        ...
        updateLayoutSize() {
            if (!this.xsheetTable || !this.container) {
                console.warn('updateLayoutSize aborted: Missing table or container.');
                return;
            }
            console.log("Updating drawing layout size...");

            // Force a reflow to ensure we have accurate dimensions
            ...
            this.layers.forEach(layer => {
                ...
                if (layer.canvas) {
                    if (layer.canvas.width !== containerWidth || layer.canvas.height !== containerHeight) {
                        ...
                        // Essential: Add high z-index to canvas elements for visibility on top of table
                        layer.canvas.style.zIndex = '9999';
                        ...
                        // Even if size hasn't changed, ensure z-index is set high
                        layer.canvas.style.zIndex = '9999';
                        layer.canvas.style.visibility = 'visible';
                    }
                } else {
        ...
        // Method to update container layout
        updateContainerLayout() {
            ...
            if (xsheetTable && printableArea && this.container) {
                ...
                if (tableRect.width > 0 && tableRect.height > 0) {
                    // Position container precisely over the table
                    this.container.style.position = 'absolute';
                    this.container.style.top = (tableRect.top - printableRect.top) + 'px';
                    this.container.style.left = (tableRect.left - printableRect.left) + 'px';
                    this.container.style.width = tableRect.width + 'px';
                    ...
                    this.container.style.zIndex = '9999'; // High z-index
                    this.container.style.visibility = 'visible';
                    this.container.style.display = 'block';
                    this.container.style.overflow = 'visible';
                    this.container.style.transform = 'none';
                    this.container.style.pointerEvents = 'none'; // Will be enabled for active tools

                    // Ensure the container has the drawing-layer-visible class for higher specificity in CSS
                    if (!this.container.classList.contains('drawing-layer-visible')) {
                        this.container.classList.add('drawing-layer-visible');
                    }
                } else {
        ...
        enableDrawing() {
            ...
            // Set container's pointer-events mode based on active tool
            if (window.XSheetApp?.Drawing?.state?.toolSystem?.activeTool) {
                const activeTool = window.XSheetApp.Drawing.state.toolSystem.activeTool;

                // For select tool, enable pointer events on container
                if (activeTool.constructor.name === 'SelectTool') {
                    this.container.style.pointerEvents = 'auto';
                } else {
                    // For drawing tools, keep container transparent to events
            ...
            // Make sure z-index is high regardless of event handling
            this.container.style.zIndex = '9999';
            this.container.style.visibility = 'visible';

            // Add drawing-active class to body for CSS targeting
        ...
        disableDrawing() {
            ...
            // Container should always have pointer-events as none when not drawing
            this.container.style.pointerEvents = 'none';
        ...
        /**
         * Additional helpful method to force drawing layer to front
         * Add this to the DrawingLayerSystem class
         */
        forceLayerToFront() {
            // Set container to front
            if (this.container) {
                this.container.style.zIndex = '9999';
                this.container.style.visibility = 'visible';
                this.container.style.display = 'block';
            ...
            // Set all canvas elements to front
            this.layers.forEach(layer => {
                if (layer.canvas) {
                    layer.canvas.style.zIndex = '9999';
                    layer.canvas.style.visibility = 'visible';
            ...
            // Redraw to ensure all elements are visible
            this.redrawAll();
        }
    ...
    class DrawingToolSystem {
        ...
        setActiveTool(toolName) {
            ...
            // Ensure layer system is properly initialized
            if (!this.layerSystem || !this.layerSystem.container) {
                console.error("Layer system not properly initialized for tool activation");
                // Try to recover the layer system
                this.layerSystem = app.Drawing.state.layerSystem;
                if (!this.layerSystem) {
                    console.error("Cannot recover layer system. Drawing tools will not work.");
                    return false;
                }
            }

            // Ensure all tools have reference to the layer system
            Object.values(this.availableTools).forEach(tool => {
                if (!tool.layerSystem) {
                    tool.layerSystem = this.layerSystem;
                }
            });

            // Continue with regular tool activation
            if (this.activeTool) {
    ...
    class PenTool extends DrawingTool {
        ...
        // Override the activate method from DrawingTool
        activate() {
            console.log("PenTool: Activating");
            this.active = true;

            // Debug the layer system
            if (!this.layerSystem) {
                console.error("PenTool: No layer system available for tool activation!");
                return;
            }

            if (!this.layerSystem.container) {
                console.error("PenTool: No layer container available! Drawing will not work.");
                return;
            }

            // Make sure the layer container is properly visible
            this.layerSystem.container.style.display = 'block';
            this.layerSystem.container.style.visibility = 'visible';
            this.layerSystem.container.style.opacity = '1';

            // Enable drawing - specifically for this tool
            this.layerSystem.enableDrawing();

            // Force the active layer's canvas to have pointer events
            const activeLayer = this.layerSystem.getActiveLayer();
            if (activeLayer && activeLayer.canvas) {
                activeLayer.canvas.style.pointerEvents = 'auto';
                console.log(`PenTool: Active layer canvas pointer-events set to 'auto'`);
            } else {
                console.error("PenTool: No active layer or canvas found");
            }

            this.attachEvents();
            console.log("PenTool: Activation complete");
        }

        attachEvents() {
            console.log("PenTool: Attaching events");
            const activeLayer = this.layerSystem.getActiveLayer();
            if (!activeLayer || !activeLayer.canvas) {
                console.error("PenTool: No active layer canvas found");
                return;
            }
            ...
            console.log(`PenTool: Canvas dimensions: ${canvas.width}x${canvas.height}`);
            console.log(`PenTool: Canvas style.pointerEvents = ${canvas.style.pointerEvents}`);
            ...
            console.log("PenTool: Events attached");
        }

        detachEvents() {
            console.log("PenTool: Detaching events");
            const activeLayer = this.layerSystem.getActiveLayer();
            if (!activeLayer || !activeLayer.canvas) {
                console.error("PenTool: No active layer canvas found for detaching events");
                return;
            }
            ...
            this.finishDrawing();
            console.log("PenTool: Events detached");
        }

        handleMouseDown(e) {
            console.log(`PenTool: Mouse down at client coordinates ${e.clientX}, ${e.clientY}`);
            e.preventDefault();
            ...
            if (e.pointerId != null) {
                try {
                    ...
                    console.error("PenTool: Error setting pointer capture:", err);
                }
            }

            const coords = this.layerSystem.screenToCanvas(e.clientX, e.clientY);
            console.log(`PenTool: Converted to canvas coordinates ${coords.x}, ${coords.y}`);
            ...
            console.log("PenTool: Started new path and added to layer");
        }
        ...
        handleMouseUp(e) {
            ...
            console.log("PenTool: Mouse up - finishing drawing");
            if (e.pointerId != null) {
                try {
                    e.target.releasePointerCapture(e.pointerId);
                } catch (err) {
                    console.error("PenTool: Error releasing pointer capture:", err);
                }
            }
            this.finishDrawing();
        }
        ...
        finishDrawing() {
            if (this.drawing && this.currentPath) {
                ...
                this.drawing = false;
                this.currentPath = null;
                console.log("PenTool: Drawing finished");
            }
        }
    }
