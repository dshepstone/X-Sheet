...
(function (app) {
    ...
    function recalculateDrawingLayerPosition(isForPrint = false, context = 'screen') {
        // ... (this function remains the same, but we'll activate the shim for print)
        if (!app.Drawing || !app.Drawing.state.layerSystem || !app.Drawing.state.layerSystem.container) {
            return;
        }
        const layerSystem = app.Drawing.state.layerSystem;
        ...
        if (context === 'print') {
            ...
            // Based on your screenshot, the drawing layer is too high. Let's try adding a few pixels.
            topPos += 4; // ADJUST THIS VALUE based on visual inspection of print output
            console.log(`[PRINT] Adjusted topPos: ${topPos.toFixed(2)} (added 4px)`);
        } else if (context === 'pdf') {
            // If PDF drawing is slightly too LOW (pushed down after audio), SUBTRACT to move it UP.
            // topPos -= 2; // ADJUST THIS VALUE if needed
        }
    ...
    function cleanupAfterExport(savedData) { // Modified order
        document.body.classList.remove('print-mode');
        // No specific printableElement style was set here, so no restore for it.
    ...
    function cleanupAfterPrint(tableData) { // Modified order
        document.body.classList.remove('print-mode');
        // No specific originalBodyClassName was used here that needs restoring besides classList.
