...
(function(app) {
    ...
    function updateTemplate() {
        const wrapper = document.getElementById('xsheet-wrapper'); // <<< GET THE WRAPPER

        if (state.currentTemplate === 'large') {
            // 11"x17" template (96 frames)
            if (wrapper) {                                     // <<< ADD CHECK
                wrapper.style.maxWidth = '11in';               // <<< TARGET WRAPPER
                // wrapper.style.maxHeight = '17in'; // Max-height on wrapper can be complex; consider if needed
            }
            document.body.style.fontSize = '9pt'; // Font size can stay on body or move to wrapper
            elements.frameCountInput.value = 96;
            state.frameCount = 96;
        } else {
            // 8"x10" template (48 frames)
            if (wrapper) {                                     // <<< ADD CHECK
                wrapper.style.maxWidth = '8in';                // <<< TARGET WRAPPER
                // wrapper.style.maxHeight = '10in';
            }
