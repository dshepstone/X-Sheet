...
(function (app) {
    ...
    function cleanupAfterExport(savedData) {
        ...
        if (app.restoreData && typeof app.restoreData === 'function') {
            console.log("Restoring core data after export...");
            app.restoreData(savedData);
        } else {
            console.error("cleanupAfterExport: app.restoreData is not available.");
        }
        if (app.Audio?.state?.audioBuffer && app.Audio.drawWaveformInCells) {
            console.log("Redrawing cell waveforms after export cleanup...");
            setTimeout(() => { // Ensure table restore is complete
                if (app.Audio?.drawWaveformInCells) app.Audio.drawWaveformInCells();
            }, 0);
        }
    ...
    function exportToPDF() {
        if (!app.collectData || !app.updateStatusMessage || !app.state) {
        ...
        if (app.Audio?.drawWaveformInCells) {
            try { app.Audio.drawWaveformInCells(); } catch (err) { console.error("Error in drawWaveformInCells:", err); }
        }
        hideUIControls();
        ...
        requestAnimationFrame(() => {
            ...
            setTimeout(() => {
                ...
                if (!targetElement) {
                    console.error("PDF Export: Printable area not found.");
                    app.updateStatusMessage('Error: Printable area missing.');
                    document.body.classList.remove('print-mode'); cleanupAfterExport(savedData); return;
                }

                console.log("Starting html2canvas capture for PDF...");
                html2canvas(targetElement, {
                    scale: 1.5, useCORS: true, logging: false, allowTaint: true,
                    backgroundColor: null, // <<< IMPORTANT FOR TRANSPARENCY
                    imageTimeout: 10000, // Increased timeout
                }).then(canvas => {
                    console.log("html2canvas capture successful for PDF.");
                    try {
                        const imgData = canvas.toDataURL('image/png');
                        if (!imgData || imgData === 'data:,') throw new Error("html2canvas returned empty image.");

                        let pdfW = app.state.currentTemplate === 'large' ? 279.4 : 215.9;
                        let pdfH = app.state.currentTemplate === 'large' ? 431.8 : 279.4;
                        ...
                        console.error('Error creating/saving PDF from canvas:', err);
                    ...
                    console.error('HTML2Canvas capture error for PDF:', err);
                    app.updateStatusMessage('Error creating PDF image: ' + err.message);
                    document.body.classList.remove('print-mode');
                    cleanupAfterExport(savedData);
                });
            }, 400); // Increased delay
    ...
    function printSheet() {
        if (!app.updateStatusMessage || !app.state) {
        ...
        requestAnimationFrame(() => {
            ...
            setTimeout(() => {
                ...
                try {
                    window.print();
                    setTimeout(() => {
                        console.log("Attempting post-print cleanup...");
                        document.body.classList.remove('print-mode');
                        cleanupAfterPrint(tableData, originalBodyStyle);
                        app.updateStatusMessage('Print process finished.');
                    }, 3000);
                } catch (printError) {
                    console.error("Error during window.print():", printError);
                    app.updateStatusMessage('Error opening print dialog.');
                    document.body.classList.remove('print-mode');
                    cleanupAfterPrint(tableData, originalBodyStyle);
                }
            }, 400); // Increased delay
