...
(function(app) {
        ...
        reader.onload = function(e) {
            ...
            state.audioContext.decodeAudioData(audioData, 
                function(buffer) {
                    ...
                    // Auto-update the frame count to fit the audio
                    autoUpdateFrameCount(frameCount);
    ...
    // Auto-update frame count to fit audio
    function autoUpdateFrameCount(frameCount) {
        // Add a buffer of 8 frames (1/3 second) for better visualization
        const neededFrames = frameCount + 8;
        
        // Make sure it's a multiple of 8 for consistency
        const frameCountAdjusted = Math.ceil(neededFrames / 8) * 8;
        
        // Only increase frame count, never decrease
        if (frameCountAdjusted > app.state.frameCount) {
            app.state.frameCount = frameCountAdjusted;
            elements.frameCountInput.value = frameCountAdjusted;
            app.generateTable(frameCountAdjusted);
            app.updateStatusMessage(`Frame count automatically updated to ${frameCountAdjusted} to fit audio`);
        } else {
            renderWaveform();
        }
    }
